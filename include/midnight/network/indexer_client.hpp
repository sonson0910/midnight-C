...
struct WalletState {
  ...
  uint64_t dust_registered_utxo_count = 0;
...
struct ContractAction {
  ...
  json unshielded_balances =
      json::array();       ///< Token balances {tokenType, amount}
...
/**
 * @brief Block information from the indexer
 */
struct BlockInfo {
  std::string hash;
  uint64_t height = 0;
 ...
 * @code
 *   IndexerClient
 * indexer("https://indexer.testnet.midnight.network/api/v4/graphql"); auto
 * state = indexer.query_contract_state("contract_addr_..."); auto wallet =
 * indexer.query_wallet_state("mn_addr_...");
 * @endcode
 */
class IndexerClient {
public:
  ...
  query_contract_balance(const std::string &contract_address);
   ...
   * @param cardano_reward_address Cardano stake address (stake_test1... or
   * stake1...)
   * @return DustGenerationStatus with full DUST generation details
   */
  DustGenerationStatus
  query_dust_generation_status(const std::string &cardano_reward_address);
   ...
   */
  std::vector<DustRegistration>
  query_dust_registrations(const std::string &cardano_reward_address);
  ...
  /**
   * @brief Query a block by height or hash
   *
   * BlockOffset in Indexer v4 GraphQL accepts either:
   *   - { height: Int } - query by block number
   *   - { hash: HexEncoded } - query by block hash
   *
  * IMPORTANT: HexEncoded values may be provided with or without the "0x" prefix.
  * The client normalizes the prefix before sending.
   *
   * @param height Block height (0 to ignore)
   * @param hash Block hash hex (empty to ignore)
   * @return BlockInfo with block details, or empty BlockInfo if not found
   */
  BlockInfo query_block(uint64_t height = 0, const std::string &hash = "");
   ...
   */
  std::vector<DustNullifierTransaction>
  query_dust_nullifiers(const std::vector<std::string> &nullifier_prefixes,
                        uint64_t from_block = 0, uint64_t to_block = 0);
  ...
  query_shielded_nullifiers(const std::vector<std::string> &nullifier_prefixes,
                            uint64_t from_block = 0, uint64_t to_block = 0);
   ...
   * @return ShieldedSession with session_id on success, empty session_id on
   * failure
   */
...
private:
  ...
  std::string get_cache_key(const std::string &prefix,
